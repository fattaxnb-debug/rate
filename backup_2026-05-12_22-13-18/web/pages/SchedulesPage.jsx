...
export default function SchedulesPage() {
  ...
  const getTemporalStatus = (schedule) => {
    if (!schedule.data_hora_agendamento) return null;
    
    const now = currentTime;
    const appointmentDate = new Date(schedule.data_hora_agendamento);
    const diffMs = appointmentDate.getTime() - now.getTime();
    const diffHours = diffMs / (1000 * 60 * 60);
    
    if (diffMs < 0) return 'atrasado'; // Já passou
    if (diffHours < 24) return 'urgente'; // Menos de 24h
    if (diffHours < 72) return 'atencao'; // Menos de 72h
    return 'normal'; // Mais de 72h
  };
  ...
  const getTemporalRowClass = (schedule) => {
    ...
    switch (status) {
      ...
      case 'urgente': return 'bg-orange-50 dark:bg-orange-950/20';
      case 'atencao': return 'bg-yellow-50 dark:bg-yellow-950/20';
      default: return '';
    }
  };
              ...
              <TableBody>
                {filteredSchedules.length === 0 ? (
                  ...
                  filteredSchedules.map((schedule) => {
                    return (
                      <TableRow key={schedule.id} className={getTemporalRowClass(schedule)}>
