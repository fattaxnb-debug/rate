...
router.get('/technicians', async (req, res) => {
  try {
    const [technicians] = await db.query('SELECT id, name, email FROM users WHERE role = "technician" ORDER BY name ASC');
...
router.post('/', async (req, res) => {
  ...
  try {
    const { client_id, equipment_id, technician_id, scheduled_date, scheduled_time, status, notes } = req.body;
    
    console.log('Extracted fields:', { client_id, equipment_id, technician_id, scheduled_date, scheduled_time });
    
    // Combinar scheduled_date e scheduled_time para formato MySQL datetime (YYYY-MM-DD HH:mm:ss)
    let finalScheduledDate = null;
    if (scheduled_date && scheduled_time) {
      finalScheduledDate = `${scheduled_date} ${scheduled_time}:00`;
    } else if (scheduled_date) {
      finalScheduledDate = `${scheduled_date} 00:00:00`;
    }
    ...
    const sql = `INSERT INTO schedules (client_id, equipment_id, technician_id, scheduled_date, status, notes) VALUES (?, ?, ?, ?, ?, ?)`;
    console.log('SQL:', sql);
    console.log('Values:', [client_id, equipment_id, technician_id, finalScheduledDate, status || 'pending', notes]);
    
    const [result] = await db.query(sql, [client_id, equipment_id, technician_id, finalScheduledDate, status || 'pending', notes]);
